...
// SMTPEmailService Real implementation of EmailService using an SMTP server
...
// NewSMTPEmailService creates a new SMTP email sender
func NewSMTPEmailService(config SMTPConfig, logger *slog.Logger) *SMTPEmailService {
...
func (sender *SMTPEmailService) SendEmailVerificationLink(ctx context.Context, email, token string) error {
	data := map[string]string{
		"VerificationLink": fmt.Sprintf("%s/api/v1/auth/verify-email?token=%s", sender.BaseURL, token),
	}

	return sender.sendEmail(ctx, email, "verification_link_template", data)
}

// SendEmailPasswordResetLink connects to the SMTP server and sends the email
func (sender *SMTPEmailService) SendEmailPasswordResetLink(ctx context.Context, email string, token string) error {
	data := map[string]string{
...
func (sender *SMTPEmailService) SendEmailVerificationCode(ctx context.Context, email string, code string) error {
	sender.logger.Info("Sending verification code to ", "email", email, "code", code)
	data := map[string]interface{}{
		"Code": code,
	}

	return sender.sendEmail(ctx, email, "verification_code_template", data)
}

// SendEmailLoginOTP connects to the SMTP server and sends the email
func (sender *SMTPEmailService) SendEmailLoginOTP(ctx context.Context, email string, code string) error {
	sender.logger.Info("Sending login otp to ", "email", email, "code", code)
	data := map[string]string{
		"Code": code,
	}
...
func (sender *SMTPEmailService) sendEmail(ctx context.Context, email string, templateName string, data any) error {
	...
	// Run the blocking call in a goroutine and listen for its
