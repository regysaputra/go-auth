...
func NewPostgresUserRepository(db *pgxpool.Pool) usecase.UserRepository {
	return &PostgresUserRepository{db: db}
}

// Save saves the user to the database
func (r *PostgresUserRepository) Save(ctx context.Context, user *domain.User) error {
	sql := "INSERT INTO users (name, email, password, verified) VALUES ($1, $2, $3, $4) RETURNING id"
	err := r.db.QueryRow(ctx, sql, user.Name, user.Email, user.Password, user.Verified).Scan(&user.ID)
...
func (r *PostgresUserRepository) FindByEmail(ctx context.Context, email string) (*domain.User, error) {
	...
	err := row.Scan(&user.ID, &user.Name, &user.Email, &user.Password, &user.Verified, &user.CreatedAt)
